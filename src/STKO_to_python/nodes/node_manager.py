...
class NodeManager:
    ...
    def _read_multi_results_all_steps(
        *,
        ...
        step_names = NodeManager._sort_step_keys(ref.keys())
...
# The legacy ``Nodes`` alias lives on the ``STKO_to_python.nodes``
# package surface (quiet) and at the deep path
# ``STKO_to_python.nodes.nodes`` (DeprecationWarning via PEP 562
# ``__getattr__`` shim). It is intentionally not declared on this
# canonical module so that ``from .node_manager import NodeManager``
# inside the library never trips a deprecation warning.
